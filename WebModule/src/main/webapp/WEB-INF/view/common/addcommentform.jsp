...
<body>
<div>
    <form name="review" commentaryId="review" method="post" action="controller">
        <fieldset>
            <legend><p>Оставьте Ваш комментарий</p></legend>
            <input type="hidden" name="command" value="addwritecomment"/>
            <input type="hidden" name="newsId" value="${news.commentaryId}"/>
            ...
            <c:if test="${commentForEdit eq null}">
                <input type="hidden" name="personId" value="${user.commentaryId}"/>
                <input type="text" maxlength="20" size="16" name="name" commentaryId="name" value="${user.name}" readonly/>
                <sup><em>*</em></sup><label for="name"><em>Name of commentary author</em></label><br>
            </c:if>
            <c:if test="${commentForEdit != null}">
                <input type="hidden" name="personId" value="${commentForEdit.personId}"/>
                <input type="hidden" name="comment_id" value="${commentForEdit.commentaryId}"/>
